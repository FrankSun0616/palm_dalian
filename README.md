...
- AI 决策防火墙：V4-Flash 输出必须通过布林位置、RSI、关键价位、证据强度和固定前向复盘检查，未通过时只展示原文，不进入执行策略与关键位计算。
...
- 每次自动分析、手动分析和页面问答都会在调用 V4-Flash 前重新请求新浪 P0/Y0 最新可用盘口；结果保存抓取时间、盘口时间和实时价格。若即时抓价失败，缓存报价不会冒充实时输入，AI 策略会被规则层阻断。
