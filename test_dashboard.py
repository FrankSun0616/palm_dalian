...
def validate_frontend() -> None:
    ...
    assert "GH_ASK_WORKFLOW_DISPATCH_URL" in app, "direct V4-Flash Q&A dispatch is missing"
    ...
    assert "直接问 V4-Flash" in html, "direct V4-Flash Q&A button copy is missing"
    assert "DeepSeek V4-Flash" in html, "V4-Flash identity is missing from the UI"
...
def validate_workflow() -> None:
    ...
    assert "live-quote-plus-cached-bars-v2" in ai_pipeline, "live-input AI pipeline marker is missing"
    assert "fetch_live_market_input" in ai_pipeline, "AI does not fetch a quote immediately before inference"
    assert '"cached_quote_used": False' in ai_pipeline, "cached quote can still be presented as live"
    assert "realtime_fetch_status" in ai_pipeline, "AI live quote status is not recorded"
    assert "fetch_live_quote" in ask_backend, "Q&A does not fetch a fresh quote"
    assert "build_context(profile_dir, realtime)" in ask_backend, "Q&A does not pass the fresh quote to V4-Flash"
